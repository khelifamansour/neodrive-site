...
export async function GET(req: Request) {
  ...
  try {
    const [{ data: topics }, { data: recentArticles }, { data: photos }] = await Promise.all([
      sb.from("marketing_topics").select("id,topic,angle,target_audience,primary_keyword,secondary_keywords,business_priority,times_used").eq("status", "idea").order("times_used", { ascending: true }).order("business_priority", { ascending: false }).limit(30),
      sb.from("seo_articles").select("slug,title,target_keyword,published_at").eq("status", "published").order("published_at", { ascending: false }).limit(12),
      sb.from("social_media_assets").select("public_url,title,context,ai_quality_score").eq("status", "ready").eq("media_type", "image").not("storage_path", "like", "generated/%").order("ai_quality_score", { ascending: false, nullsFirst: false }).limit(12),
    ]);

    const recentTitles = new Set((recentArticles || []).flatMap((item: any) => [String(item.title || "").toLowerCase(), String(item.target_keyword || "").toLowerCase()]));
    const availableTopics = (topics || []).filter((item: any) => !recentTitles.has(String(item.topic || "").toLowerCase()));
    const pool = availableTopics.length ? availableTopics : (topics || []);
    const chosen = requestedTopic
      ? topics?.find((item: any) => String(item.topic || "").toLowerCase() === requestedTopic.toLowerCase())
      : pool[Math.floor(Date.now() / (86400000 * 7)) % (pool.length || 1)];
    ...
    const image = photo?.public_url
      ...
      : "";
    const relatedArticles = (recentArticles || []).slice(0, 3).map((item: any) => `<li><a href="/blog/${safeHtml(String(item.slug))}">${safeHtml(String(item.title))}</a></li>`).join("");
    const links = relatedArticles ? `<section><h2>Autres guides NeoDrive</h2><ul>${relatedArticles}</ul></section>` : "";
    const content = `${image}${String(out.content || "")}${links}<p><a href="https://wa.me/33628261446">Demander les photos et les tarifs NeoDrive sur WhatsApp</a></p>`;
    ...
    return NextResponse.json({ ok: true, article: data, url: articleUrl, triggerSource, topicSource: requestedTopic ? "manual" : chosen ? "marketing_topics" : "fallback" });
  } catch (e) {
    return fail(e, 500);
  }
}
