...
function mediaElement(a:any,i:number){const base:any={track:1,time:i*3,duration:3,source:a.public_url,width:"100%",height:"100%",x:"50%",y:"50%",x_anchor:"50%",y_anchor:"50%",fit:"cover"};return {...base,type:isVideo(a)?"video":"image",animations:i? [{time:0,duration:.35,transition:true,type:"fade"}]:[]};}
...
export async function GET(req:Request){
 ...
 const theme=angles[Math.floor(Date.now()/86400000)%angles.length];let hook=theme==="prix accessible"?"Une voiture sans permis neuve à partir de 3 990 €":theme==="recharge 220 V"?"Une prise 220 V. Et c'est parti.":theme==="look compact"?"Petite dehors. Vraie présence sur la route.":"NeoDrive dans la vraie vie";
 if(process.env.OPENAI_API_KEY)try{const c=new OpenAI({apiKey:process.env.OPENAI_API_KEY});const r=await c.responses.create({model:process.env.OPENAI_SOCIAL_MODEL||"gpt-4.1-mini",input:`Écris un hook vidéo vertical très court (max 9 mots), naturel et crédible pour NeoDrive. Angle: ${theme}. Faits sûrs: voiture sans permis électrique 2 places, gamme à partir de 3 990 € TTC, recharge prise 220 V. Pas de promesse de sécurité, autonomie, stock, garantie ou droit de conduire. Retourne uniquement le hook.`,max_output_tokens:50});hook=r.output_text.trim()||hook;}catch{}
 ...
 return NextResponse.json({ok:true,jobId:job.id,renderId:render?.id||null,theme,hook,assets:chosen.length});
}
