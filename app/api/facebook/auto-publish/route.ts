...
function isVideoAsset(asset:Asset){return asset.media_type==="reel"||asset.media_type==="video"||/\.(mp4|mov|m4v)(\?|$)/i.test(asset.public_url||"");}
...
async function publish(pageId:string,token:string,asset:Asset,message:string){
  const isVideo=isVideoAsset(asset);
  const endpoint=`https://graph.facebook.com/v26.0/${pageId}/${isVideo?"videos":"photos"}`;
  const body=isVideo
    ? new URLSearchParams({file_url:asset.public_url,description:message,access_token:token})
    : new URLSearchParams({url:asset.public_url,caption:message,published:"true",access_token:token});
  const r=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body,cache:"no-store"});
  const j=await r.json().catch(()=>({}));
  if(!r.ok||(!j.id&&!j.post_id)){
    const e=j?.error||{};
    throw new Error(`Facebook publish failed: ${e.message||"Unknown error"}${e.code?` (code ${e.code}${e.error_subcode?`, subcode ${e.error_subcode}`:""})`:""}; média=${isVideo?"video":"image"}`);
  }
