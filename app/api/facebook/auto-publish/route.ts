...
export async function GET(request:Request){const force=new URL(request.url).searchParams.get("force")==="1";const secret=process.env.CRON_SECRET;const auth=request.headers.get("authorization");if(!secret||auth!==`Bearer ${secret}`)return NextResponse.json({ok:false,error:"Unauthorized"},{status:401});if(!force&&!parisSlot())return NextResponse.json({ok:true,skipped:true,reason:"Outside Paris publishing window"});const token=process.env.FACEBOOK_PAGE_ACCESS_TOKEN;const pageId=process.env.FACEBOOK_PAGE_ID;if(!token||!pageId)return NextResponse.json({ok:false,error:"FACEBOOK_PAGE_ACCESS_TOKEN or FACEBOOK_PAGE_ID missing"},{status:503});const asset=await selectAsset();if(!asset)return NextResponse.json({ok:true,skipped:true,reason:"No ready media asset"});try{const message=await caption(asset);const postId=await publish(pageId,token,asset,message);await mark(asset.id);return NextResponse.json({ok:true,published:true,postId,media:asset.public_url,mediaType:asset.media_type,forced:force});}catch(e){return NextResponse.json({ok:false,error:e instanceof Error?e.message:"Unknown error"},{status:500});}}
