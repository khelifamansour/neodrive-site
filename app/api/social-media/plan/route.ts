...
export async function GET(req:Request){
 ...
 const [ar,tr]=await Promise.all([fetch(`${SB}/rest/v1/social_media_assets?status=eq.ready&select=id,public_url,media_type,title,context,times_used,priority,ai_quality_score&order=priority.desc,ai_quality_score.desc.nullslast,times_used.asc.nullsfirst,last_used_at.asc.nullsfirst&limit=50`,{headers:H(k),cache:"no-store"}),fetch(`${SB}/rest/v1/marketing_topics?status=eq.idea&select=id,topic,angle,pillar,target_audience,business_priority,times_used&order=business_priority.desc,times_used.asc&limit=100`,{headers:H(k),cache:"no-store"})]);
 ...
 const vids=assets.filter(isVideo),imgs=assets.filter((a:any)=>!isVideo(a));const c=process.env.OPENAI_API_KEY?new OpenAI({apiKey:process.env.OPENAI_API_KEY}):null;const rows:any[]=[];
 for(let day=0;day<4;day++)for(let s=0;s<3;s++){
  ...
  if(c)try{const format=["question-réponse pédagogique","mini-guide pratique","comparatif factuel et nuancé","coulisses de la marque","conseil budget","idée reçue expliquée","scénario de vie quotidienne"][(day*3+s)%7];const r=await c.responses.create({model:process.env.OPENAI_SOCIAL_MODEL||"gpt-4.1-mini",input:`Légende sociale française originale pour un vrai média NeoDrive. Format:${format}. Sujet:${hook}. Angle:${topic?.angle||"usage réel"}. Pilier:${topic?.pillar||"mobilité"}. Média:${asset.title||asset.media_type}. Contexte:${asset.context||"aucun"}. Faits sûrs: VSP électrique 2 places, à partir de 3 990 € TTC, recharge 220 V, livraison France. Réglementation: quadricycle léger accessible à partir de 14 ans avec permis AM lorsque requis; avant 14 ans non autorisé; personnes nées avant 1988 cas particulier à vérifier auprès de Service-Public. Comparatifs Citroën Ami, Fiat Topolino, Aixam: rester factuel, jamais dénigrant, ne jamais inventer prix ou défauts. Acier: ne l’affirmer que si le média ou contexte le confirme. 3-6 phrases naturelles, angle réellement différent, 3-6 hashtags, CTA discret. Ne jamais inventer autonomie, sécurité, stock, garantie, avis ou droit de conduire. Retourne seulement la légende.`,max_output_tokens:350});caption=r.output_text.trim()||caption;}catch{}
