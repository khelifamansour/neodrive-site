...
export async function POST(req: Request) {
  ...
  for (const a of dynamicArticles || []) {
    ...
    rows.push({
      ...
      url: `${SITE}/blog/${slug}`,
      metrics: metricMap.get(slug) || { ...EMPTY_METRICS },
    });
  }
  ...
  const importantPages = [
    ["/", "Accueil"],
    ["/voiture-sans-permis", "Hub voiture sans permis"],
    ["/prix-voiture-sans-permis", "Prix voiture sans permis"],
    ["/voiture-sans-permis-occasion", "Voiture sans permis occasion"],
    ["/voiture-sans-permis-electrique", "Voiture sans permis électrique"],
    ["/citroen-ami-ou-neodrive", "Citroën Ami ou NeoDrive"],
    ["/fiat-topolino-ou-neodrive", "Fiat Topolino ou NeoDrive"],
    ["/videos", "Vidéos réelles NeoDrive"],
    ["/produit", "Page produit"],
    ...["toulouse","paris","lyon","marseille","bordeaux","lille","nantes","strasbourg","montpellier","nice","rennes","grenoble"].map(city => [`/voiture-sans-permis/${city}`, `Voiture sans permis ${city.charAt(0).toUpperCase()}${city.slice(1)}`]),
  ] as [string, string][];
  ...
  const videoRows = (videos || []).map((v: any) => {
    ...
    return {
      ...v,
      generated: String(v.storage_path || "").startsWith("generated/"),
      page_url: `${SITE}/videos/${v.id}`,
      metrics: metricMap.get(key) || { ...EMPTY_METRICS },
    };
  });
  ...
  const socialByPlatform = ["facebook", "instagram", "tiktok"].map(platform => ({
    platform,
    published: publishedSocial.filter((q: any) => q.platform === platform).length,
    failed: failedSocial.filter((q: any) => q.platform === platform).length,
    scheduled: scheduledSocial.filter((q: any) => q.platform === platform).length,
  }));
  ...
  return NextResponse.json({
    ...
    cron: {
      schedule: "15 5 * * 1,3,5",
      description: "Lundi, mercredi et vendredi à 05:15 UTC (07:15 en France actuellement)",
      next_run_at: nextSeoRunIso(),
      social_description: "Instagram 07:30, 11:30 et 16:30 UTC; Facebook 5 minutes après chaque créneau",
    },
    summary: {
      ...
      article_views_30d: articleViews30,
      page_views_30d: pageViews30,
      video_views_30d: videoViews30,
      total_views_30d: articleViews30 + pageViews30 + videoViews30,
      google_views_30d: totalGoogle30,
      seo_cta_clicks_30d: totalCta30,
      total_videos: videoRows.length,
      original_videos: videoRows.filter((v: any) => !v.generated).length,
      generated_videos: videoRows.filter((v: any) => v.generated).length,
      social_published_last_7d: socialLast7,
      social_failed: failedSocial.length,
      social_scheduled: scheduledSocial.length,
      article_google_30d: articleGoogle30,
      article_cta_30d: articleClicks30,
    },
    articles: articles.slice(0, 180),
    pages: pageRows.slice(0, 120),
    videos: videoRows.slice(0, 100),
    social: {
      by_platform: socialByPlatform,
      recent: (queue || []).slice(0, 100),
    },
    video_jobs: videoJobs || [],
    runs: runs || [],
    search_console: {
      connected: false,
      note: "Le tableau mesure déjà les visites, provenance Google et clics. Pour impressions, requêtes, CTR et position moyenne, connecter Google Search Console reste nécessaire.",
    },
  });
}
