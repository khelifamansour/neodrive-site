...
export default function Page() {
  ...
  const [videoAction, setVideoAction] = useState<"create"|"publish"|null>(null);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      let cursor = 0; const done: UploadInfo[] = [];
      async function worker() { while (true) { const n = cursor++; if (n >= fs.length) return; const result = await uploadOne(fs[n], j.uploads[n] || { index: n, error: "Réponse manquante" }, n); if (result) done.push(result); setStatus(`Traitement : ${Math.min(cursor, fs.length)}/${fs.length} — ${done.length} envoyé(s)`); } }
      await Promise.all([worker(), worker(), worker()]);
      if (done.length) { const rr = await fetch("/api/social-media/register", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ passcode, batchContext: ctx, items: done }) }); const jj = await rr.json(); if (!rr.ok || !jj.ok) throw new Error(jj.error || "Enregistrement incomplet"); }
  ...
  async function publishNow(platform: "instagram" | "facebook") {
    ...
    setPublishing(platform); setStatus(`Publication ${platform === "instagram" ? "Instagram" : "Facebook"} en cours…`);
  ...
  async function createVideoNow(){
    if(!passcode){setStatus("❌ Entre d’abord le code d’accès.");return;}
    setVideoAction("create"); setStatus("🎬 Création de la compilation vidéo en cours…");
    try{const r=await fetch("/api/video/manual-create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({passcode})});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||j.reason||"Création impossible");if(j.setupNeeded)throw new Error("Il manque CREATOMATE_API_KEY dans Vercel");setStatus(`✅ Vidéo lancée — thème : ${j.theme||"NeoDrive"}. Le rendu prend généralement quelques instants. Ensuite utilise le bouton de publication ci-dessous.`);}
    catch(e){setStatus(`❌ ${e instanceof Error?e.message:"Erreur vidéo"}`);}finally{setVideoAction(null);}
  }

  async function publishLatestVideo(){
    if(!passcode){setStatus("❌ Entre d’abord le code d’accès.");return;}
    setVideoAction("publish"); setStatus("🚀 Publication de la dernière vidéo sur Instagram + Facebook…");
    try{const r=await fetch("/api/video/publish-latest",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({passcode})});const j=await r.json();if(!r.ok&&r.status!==207)throw new Error(j.error||"Publication vidéo impossible");const ig=j.instagram?.published?"Instagram ✅":"Instagram ⚠️";const fb=j.facebook?.published?"Facebook ✅":"Facebook ⚠️";setStatus(`🎬 ${ig} · ${fb}${j.video?" — dernière compilation utilisée":""}`);}
  ...
  return <main style={{ maxWidth: 760, margin: "0 auto", padding: "28px 18px 60px", fontFamily: "Arial,sans-serif" }}>
    ...
    <p>Ajoute tes vraies photos et vidéos en lot. Les médias servent ensuite aux publications et aux compilations vidéo automatiques.</p>
    ...
    <section style={panel}>
      <h2 style={{marginTop:0}}>🎬 Vidéos automatiques</h2>
      <p>Fabrique une compilation verticale avec tes vraies photos/vidéos, le prix « À partir de 3 990 € TTC » et le site NeoDrive. Une compilation est aussi programmée automatiquement une fois par jour.</p>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <button onClick={createVideoNow} disabled={!!videoAction} style={{...button,background:"#7c3aed"}}>{videoAction==="create"?"Création…":"Créer une vidéo maintenant"}</button>
        <button onClick={publishLatestVideo} disabled={!!videoAction} style={{...button,background:"#0f766e"}}>{videoAction==="publish"?"Publication…":"Publier la dernière vidéo sur Instagram + Facebook"}</button>
      </div>
      <p style={{fontSize:13,color:"#666"}}>Après « Créer une vidéo », attends que le rendu soit terminé avant de publier. Si aucune vidéo terminée n’est disponible, le bouton te le dira.</p>
    </section>

    <hr style={{ margin: "32px 0" }} />
    <h2>Publication supplémentaire</h2>
    <p>Instagram et Facebook continuent à publier automatiquement. Utilise ces boutons pour ajouter un post immédiat.</p>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
